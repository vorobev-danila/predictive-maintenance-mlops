...
def load_and_prepare_data(data_path='src/data/datasets/CMAPSSData'):
    ...
    test = pd.read_csv(os.path.join(data_path, 'test_FD001.txt'),
                       sep='\s+', header=None, names=columns)
    rul = pd.read_csv(os.path.join(data_path, 'RUL_FD001.txt'),
    ...
    print(f"Обучающая выборка: {train_with_rul.shape[0]} строк, {train_with_rul.shape[1]} колонок")
